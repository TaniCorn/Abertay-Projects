...
void BaseComputeShader::CreateComputeData()
{
    ...
    hr = CreateConstantBuffer(&cb_FaultLines, sizeof(Faulting));
    assert(hr == S_OK);
...
//    // This is used for reading the buffer during the transpose
    ...
    hr = device->CreateShaderResourceView(g_pBuffer2, &srvbuffer_desc, &g_pBuffer2SRV);
//#if defined(_DEBUG)
...
//#endif
    ...
    hr = device->CreateUnorderedAccessView(g_pBuffer1, &uavbuffer_desc, &g_pBuffer1UAV);
...
void BaseComputeShader::GPUSort(ID3D11DeviceContext* currentContext)
{
    fin = false;
        ...
        currentContext->UpdateSubresource(g_pBuffer2, 0, nullptr, &data[0], 0, 0);//puts all of data into buffer1
            ...
            // Sort the row data
            currentContext->CSSetShaderResources(0, 1, &g_pBuffer2SRV);
            currentContext->CSSetUnorderedAccessViews(0, 1, &g_pBuffer1UAV, nullptr);
            currentContext->CSSetShader(g_pComputeShaderBitonic, nullptr, 0);
            currentContext->Dispatch(1, 1, 1);//This means that there will be 2 threads that act upon this
